...
module.exports = {
  ...
  migrateIndex: async (index, source, destination, script) => {
  ...
  migrate: async (source, destination, indexRegex = ".*", script) => {
    ...
    for (let i = 0; i < indexesOnSourceToCopy.length; i++) {
      limiter.take(async () => {
        ...
        const result = await es.migrateIndex(indexesOnSourceToCopy[i], source, destination, script);
        const indexData = srcIndexes.find((srcIndex) => {
